...
public class AccessFilter extends BaseFilter {
    ...
    public Object run() throws ZuulException {
        ...
        // 第5步：如果包含当前的权限标识符，则从zuul
        // header中取出用户id，根据用户id取出缓存中的用户拥有的权限，如果没有取到则通过Feign调用权限服务获取并放入缓存，判断用户拥有的权限是否包含当前请求的权限标识符
        String userId = RequestContext.getCurrentContext().getZuulRequestHeaders()

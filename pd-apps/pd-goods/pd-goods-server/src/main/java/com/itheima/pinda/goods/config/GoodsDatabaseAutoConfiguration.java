 ...
 * 一个服务如何配置多数据源？
 * <p>
 * 1. 本类中的 basePackages = {"com.itheima.pinda"}, 修改为 basePackages =
 * {"com.itheima.pinda.product.dao"} # 这个路径根据你的情况修改
 * 2. 修改本类中的 @ConfigurationProperties(prefix = "spring.datasource.druid")
 * 为 @ConfigurationProperties(prefix = "spring.datasource.druid.master")
 * <p>
 * 3. 复制该类 重命名为 SlaveDatabaseAutoConfiguration
 * 4. 修改 DATABASE_PREFIX 为 slave
 * 5. 修改 @ConfigurationProperties(prefix = "spring.datasource.druid")
 * 为 @ConfigurationProperties(prefix = "spring.datasource.druid.slave")
 * 6. SlaveDatabaseAutoConfiguration 中的 basePackages = {"com.itheima.pinda"},
 * 修改为 basePackages = {"com.itheima.pinda.order.dao"} # 这个路径根据你的情况修改
 * 7. mysql.yml 中增加2个数据源的配置 spring.database.druid.master 和
 * spring.database.druid.slave
 * <p>
 * 完成上述修改后， 位于com.itheima.pinda.product.dao 包下的dao 将操作 master 库，
 * com.itheima.pinda.order.dao中的dao 将操作slave库
 */
@Configuration
@Slf4j
@MapperScan(basePackages = {
        "com.itheima.pinda", }, annotationClass = Repository.class, sqlSessionFactoryRef = GoodsDatabaseAutoConfiguration.DATABASE_PREFIX
                + "SqlSessionFactory")
...
public class GoodsDatabaseAutoConfiguration extends BaseDatabaseConfiguration {
    ...
    public SqlSessionTemplate getSqlSessionTemplate(
            @Qualifier(DATABASE_PREFIX + "SqlSessionFactory") SqlSessionFactory sqlSessionFactory) {
    ...
    public DataSourceTransactionManager dsTransactionManager(
            @Qualifier(DATABASE_PREFIX + "DataSource") DataSource dataSource) {
    ...
    public TransactionInterceptor transactionInterceptor(
            @Qualifier(DATABASE_PREFIX + "TransactionManager") PlatformTransactionManager transactionManager) {
